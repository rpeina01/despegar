...
while to <= end_date
    ...
    while to_aux <= (to+one_day*margin)
        threads.push(Thread.new(to_aux){ |to_aux|
            url = "http://www.despegar.cl/shop/flights/data/search/roundtrip/scl/MIA/#{from.year}-#{from.month}-#{from.day}/#{to_aux.year}-#{to_aux.month}-#{to_aux.day}/1/0/0/TOTALFARE/ASCENDING/NA/NA/NA/NA/NA"
            uri = URI(url)
